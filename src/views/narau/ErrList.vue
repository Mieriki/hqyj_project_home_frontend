...
<!--      </el-button>-->
  ...
  <el-table :data="mondaiList" border :header-cell-class-name="headerBg" @selection-change="handleSelectionChange"
    ...
    <el-table-column label="错误次数" prop="errorCount" width="250">
      <template #default="scope">
        <el-tag type="danger" effect="dark">{{ scope.row.errorCount }}</el-tag>
      </template>
    </el-table-column>
...
function initializePage() {
  post(`/mondais/get/error/mondai/all`, searchValue, (data: any) => {
...
function handleDelete(row) {
  get(`/errorQuestions/delete/${row.id}`, () => {
...
function handleDeleteList() {
  post(`/errorQuestions/delete`, selectedRowList.value.map(row => row.id), () => {
    ElMessage.success('删除成功!')
    initializePage()
  })
}
