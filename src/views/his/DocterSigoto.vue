...
<!--      </el-button>-->
  ...
  <el-scrollbar max-height="75vh">
    <el-card v-for="item in docterList" style="margin-bottom: 10px;" :key="item.id">
      <el-row>
        <el-col :span="4">
          <el-row>
            <el-avatar :size="60" :src="item.picture" style="margin: auto 20px auto 10px;"></el-avatar>
            <div style="">
              <p style="font-size: 16px; font-weight: bold;">姓名: {{ item.username }}</p>
              <p style="font-size: 16px; font-weight: bold;">科室:
                {{ deptList.find(dept => dept.id === item.deptId)?.deptName }}</p>
            </div>
          </el-row>
          <el-button type="primary" @click="handleEdit(item)" style="width: 80%;">编辑</el-button>
        </el-col>
        <el-col :span="20">
              ...
              >
                ...
                >
                  <div class="cell-content">
                    {{ getSchedulingInfo(item.id, day, timeSlot) }}
                  </div>
                </td>
              </tr>
            </table>
          ...
          >
            <div class="cell-content">
              <el-select v-model="getUpdataScheduling(doctor.id, day, timeSlot).schedulingType">
                ...
                <el-option v-for="registered in registeredList" :key="registered.id" :label="registered.regName" :value="registered.regName"></el-option>
              </el-select>
            </div>
          </td>
        </tr>
...
const doctorSchedulingInfo = computed(() => {
  return schedulingList.value.reduce((acc, scheduling) => {
    if (!acc[scheduling.userId]) {
      acc[scheduling.userId] = {};
    }
    if (!acc[scheduling.userId][scheduling.schedulingDay]) {
      acc[scheduling.userId][scheduling.schedulingDay] = {};
    }
    acc[scheduling.userId][scheduling.schedulingDay][scheduling.subsectionType] = scheduling.schedulingType;
    return acc;
  }, {});
});

function getSchedulingInfo(userId, schedulingDay, subsectionType) {
  const schedulingType = doctorSchedulingInfo.value[userId]?.[schedulingDay]?.[subsectionType];
...
function initializePage() {
  get(`/sso/users/get/doctors?name=${searchValue.name}`, (data: any) => {
    docterList.value = data
  })
  get(`/sso/depts/get`, (data) => {
    deptList.value = data
  })
  get(`/his/schedulings/get`, (data) => {
    schedulingList.value = data
  })
  get(`/his/registereds/get`, (data: any) => {
    //筛选名字里没有"+"号的
    registeredList.value = data.filter(registered => registered.regName.indexOf("+") === -1);
...
function getScheduling(userId, schedulingDay, subsectionType) {
