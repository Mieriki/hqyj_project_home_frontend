...
<!--      <el-button type="primary" style="margin-left: 5px; width: 80px; height: 32px;" size="small" @click="exportData">导出<el-icon><Upload /></el-icon></el-button>-->
  ...
  <el-table :data="purchaseList" border :row-class-name="tableRowClassName" @selection-change="handleSelectionChange" max-height=525>
    ...
    <el-table-column prop="auditMsg" label="审核信息" width="200"></el-table-column>
...
function handleEdit(row) {
  ...
  infoList.value = row.flowInfoList
  console.log('flowInfoList', row.flowInfoList)
  if (purchases.status === 'iti-sinsatu') {
    akusyon.value = 1
  } else if (purchases.status === 'ni-sinsatu') {
    akusyon.value = 2
  } else {
    akusyon.value = 3
  }
