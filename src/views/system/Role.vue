<template>
  <div style="margin-bottom: 10px;">
    <el-row>
      <el-input v-model="searchValue.name" style="width: 240px;" size="small" placeholder="请输入角色或角色名称"
                type="text">
        <template #prepend>
  ...
  <el-table :data="roleList" border @selection-change="handleSelectionChange" max-height=525>
    ...
    <el-table-column label="操作" fixed="right">
      <template #default="scope">
        <el-button size="small" type="primary" @click="handleRole(scope.row)">分配权限
          <el-icon>
            <Avatar/>
          </el-icon>
        </el-button>
  ...
  <el-dialog
    ...
    />
    <el-button type="primary" style="width: 200px; margin-top: 20px; margin-left: 5px;" @click="roleSubmitForm">确认
    </el-button>
    <el-button type="info" style="width: 200px; margin-top: 20px; margin-left: 60px;" @click="handleClose">取消
    </el-button>
...
function exportData() {
  ...
  fetch(`http://mieriki.net/mugen/api/sso/roles/get/excel`, {
    method: 'GET',
    headers: headers // 确保accessHeader()返回正确的headers对象
      ...
      .then(blob => {
        ...
        a.href = url;
        a.download = `Role_${new Date().toLocaleString()}.xlsx`; // 设置文件名
