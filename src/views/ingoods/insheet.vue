<template>
	...
	<el-table :data="cartList" border :header-cell-class-name="headerBg" @selection-change="handleSelectionChange"
		...
		<el-table-column prop="purchaseDate" label="进货日期" width="160">
			<template #default="{ row }">
				{{ formatDate(row.purchaseDate) }}
			</template>
		</el-table-column>
		
		<el-table-column prop="purchaseNumber" label="进货单号" width="180">
		</el-table-column>
...
<script setup lang="ts">
	...
	function initializePage() {
		post(`/purchaseLists/get`, searchValue, (data) => {
			cartList.value = data.cartList.map(cart => {
				cart.pay = cart.state === 1 ? true : false
				cart.purchaseDate = new Date(cart.purchaseDate).toLocaleDateString('zh-CN')
				return cart
			});
