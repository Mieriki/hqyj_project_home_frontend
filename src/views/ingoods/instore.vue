<template>
	...
	<el-dialog
		...
		<el-form :model="cart" :rules="rules" ref="formRef" label-width="100px">
			<el-form-item label="入库单据" prop="purchaseNumber" style="width: 500px;">
				<el-input v-model="cart.purchaseNumber"></el-input>
			</el-form-item>
			<el-form-item label="供应商" prop="supplierId" style="width: 500px;">
				<el-select v-model="cart.supplierId" placeholder="无" filterable>
...
<script setup lang="ts">
	...
	function initializePage() {
		post(`/goods/get/cart`,searchValue , (data: any) => {
			goodsList.value = data.goodsList
			count.value = data.count
		})
		get(`/suppliers/get`, (data:　any) => {
	...
	function nextAdd(row: any) {
	    const exists = cart.goodsInList.some(goods => goods.id === row.id);
	    if (exists) {
	...
	function addSubmitForm() {
	    formRef.value.validate((valid) => {
	        ...
	        if (valid) {
				...
				post(`/purchaseLists/post/cart`, cart, () => {
